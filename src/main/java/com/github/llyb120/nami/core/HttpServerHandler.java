...
//    //dev-end
//
//    public void writeObject(ChannelHandlerContext ctx, HttpResponse response, boolean keepAlive) {
...
//            ctx.channel().writeObject(new PongWebSocketFrame(frame.content().retain()));
...
//            ctx.writeObject(new DefaultFullHttpResponse(HttpVersion.HTTP_1_1, HttpResponseStatus.CONTINUE));
//        }
//
...
//            writeObject(ctx, response, false);
//            return;
//        }
//
...
//                    Object[] args = Param.AutoWiredParams(clz, method, response, null);
//                    if (route.aops != null) {
...
//
//        response.setStatus(NOT_FOUND);
//        writeObject(ctx, response, keepAlive);
//    }
//
//    @Override
...
////                    sendFileFuture = ctx.writeObject(new ChunkedNioFile(multipartFile.file), ctx.newProgressivePromise());
////                } else {
...
////                    throw new IOException();
////                }
////                // HttpChunkedInput will writeObject the end marker (LastHttpContent) for us.
